...
module Ivar = struct
  ...
  let fill t x =
    ...
    | Empty q -> t.state <- Full x ; Queue.iter (fun f -> f x) q
...
let create_process prgn =
  ...
  | 0 ->
     ...
     ( try
         ...
         flush oc ; Unix.close out1 ; exit 0
       with _ -> exit 127 )
  | pid ->
     Unix.close out1 ;
     out0, pid
...
let restart_throttle () =
  while Hashtbl.length running < concurrency
        && (not (Queue.is_empty waiting_for_slot))
  do Ivar.fill (Queue.pop waiting_for_slot) () done
...
let run fiber =
  ...
  let rec loop () =
    ...
    then ( let pid, status = Unix.wait () in
